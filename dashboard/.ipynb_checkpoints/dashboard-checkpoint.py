...
season_sel = st.sidebar.multiselect("Season", list(season_map.values()), default=list(season_map.values()))
workingday_sel = st.sidebar.selectbox("Working Day", ["All", "Working Day Only", "Weekend/Holiday"])
...
season_daily = day_f.groupby('season_name', as_index=False)['cnt'].mean().sort_values('cnt', ascending=False)
